...
void removeSymbol(char symbol[], int& size, char toRemove) {
    for (int i = 0; i < size; ++i) {
        if (symbol[i] == toRemove) {
...
void printSymbols(const char symbol[], int size) {
    for (int i = 0; i < size; ++i) {
        cout << symbol[i];
        if (i < size - 1) {
            cout << ",";
...
int main() {
    ...
    removeSymbol(symbol, size, first);

    cout << "Enter 2nd symbol to use (";
    printSymbols(symbol, size);
    cout << "): ";
    cin >> second;
    ...
    removeSymbol(symbol, size, second);
